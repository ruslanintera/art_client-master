...
const Obj = observer(() => {
  ...
  useEffect(() => {
    fetchOnePhoto(id).then((data) => {
      device.setPhotoOne(data)
      if (!device.getPhotoOne) return
  ...
  function DELETE(event) {
    fetchPhotoDelete(oneValue.id)
    history.push(PHOTO_ROUTE + "/")
  }
  function UPDATE(event) {
    fetchPhotoUpdate(oneValue)
  }
  const onDone = (data) => {
  ...
  const upload_MP4 = () => {
    ...
    fetchPhotoUploadMP4(formData, oneValue.id).then((data) =>
      onDone(data)
    )
  }
  const upload_JPG = () => {
    ...
    fetchPhotoUploadJPG(formData, oneValue.id).then((data) =>
      onDone(data)
    )
  }
